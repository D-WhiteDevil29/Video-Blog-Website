...
import ScannerSound from '../audio/scanner.wav';
...
const Create = () => {
  ...
  const uploadDetails = async () =>{
    try{
      setLoading(true);
      if(!title && !category && !videoAsset && !location){
        setAlert(true);
        setAlertStatus("error");
        setAlertIcon(<IoWarning fontSize={25} />);
              ...
              >
                <Menu>
                    <MenuButton
                        ...
                        colorScheme='blue' 
                        as={Button} 
                        rightIcon={<IoChevronDown fontSize={{base:15,lg:20}} />}
                    >
                        {category}
                    </MenuButton>
                    <MenuList 
